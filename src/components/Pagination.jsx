...
export default class Pagination extends React.Component {
  ...
  componentWillUpdate({ paginate: { current, next, last } }) {
  ...
  render() {
    return (
      <div className='pagination-container full-width flex-center wrap'>
        <p className={`button prev five-color four-background${this.current > 1 ? '' : ' disabled'}`} onClick={this.goFirst}>First</p>
        <div className={`arrow four-background prev${this.current > 1 ? '' : ' disabled'}`} onClick={this.goBack}>
          <Arrow className='prev' />
        </div>
        <p className='pages four-background five-color'>{`Page ${this.current} of ${this.last}`}</p>
        <div className={`arrow four-background next${this.current < this.last ? '' : ' disabled'}`} onClick={this.goNext}>
          <Arrow className='next' />
        </div>
        <p className={`button next four-background five-color${this.current < this.last ? '' : ' disabled'}`} onClick={this.goLast}>Last</p>
      </div>
    )
  }
}
